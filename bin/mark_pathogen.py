#!/usr/bin/env python3
"""
Utility to condense EMU combined counts into key columns.

Reads the TSV produced by `emu combine-outputs ... --counts` and returns a table
containing `tax_id`, `abundance`, `species`, `genus`, and `estimated counts`.
"""

from __future__ import annotations

import argparse
import sys
from pathlib import Path

import numpy as np
import pandas as pd

VERSION="0.1.0-DEV"
CZID_BAC_VER="v0.2.1"

TAXONOMY_COLUMNS = {
    "tax_id",
    "species",
    "genus",
    "family",
    "order",
    "class",
    "phylum",
    "clade",
    "superkingdom",
    "subspecies",
    "species subgroup",
    "species group",
}

value_columns = ["abundance", "estimated counts"]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Select and summarise columns from an EMU combined counts TSV.\n\n"
            "The script expects the input generated with "
            "`emu abundance --counts`."
        ),
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "input_tsv",
        type=Path,
        help="Path to abundance file",
    )
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
        default=Path("-"),
        help="Output TSV file path or '-' for stdout",
    )
    parser.add_argument(
        "-v", "--version",
        action="version",
        version=f"%(prog)s {VERSION}",
        help="Print program version and exit",
    )
    return parser.parse_args()


def main() -> int:
    args = parse_args()

    try:
        df = pd.read_csv(args.input_tsv, sep="\t")
    except FileNotFoundError:
        sys.stderr.write(f"[error] Input file not found: {args.input_tsv}\n")
        return 1

    # More check, because chatgpt sayso
    # TODO: KMB this.
    counts = df[value_columns].apply(pd.to_numeric, errors="coerce").fillna(0)

    df["estimated counts"] = counts.sum(axis=1)

    total_counts = df["estimated counts"].sum()
    if total_counts > 0:
        df["abundance"] = df["estimated counts"] / total_counts
    else:
        df["abundance"] = 0.0

    output_columns = ["tax_id", "species", "genus", "estimated counts", "abundance" ]
    missing_columns = [col for col in output_columns if col not in df.columns]
    if missing_columns:
        sys.stderr.write(
            f"[error] Missing expected columns after processing: {missing_columns}\n"
        )
        return 1


    # Flag rows where the tax_id is in the CZID known pathogen list.
    result = df[output_columns].copy()
    result["known pathogen"] = np.where(result["tax_id"].isin(CZID_BAC_SET), "CZID_BAC", "---")

    destination = args.output
    if destination == Path("-"):
        result.to_csv(sys.stdout, sep="\t", index=False)
    else:
        result.to_csv(destination, sep="\t", index=False)

    return 0


# Last Updated: 2024-06-05T22:19:05-07:00. CZ ID Pathogen List v0.2.1.
CZID_BAC_LIST = [
    "46125",  # Abiotrophia defectiva
    "85698",  # Achromobacter xylosoxidans
    "470",  # Acinetobacter baumannii
    "1993",  # Actinomadura madurae
    "111805",  # Actinomadura pelletieri
    "52769",  # Actinomyces gerencseriae
    "1659",  # Actinomyces israelii
    "59505",  # Actinotignum schaalii
    "119206",  # Aerococcus sanguinicola
    "1376",  # Aerococcus urinae
    "648",  # Aeromonas caviae
    "644",  # Aeromonas hydrophila
    "654",  # Aeromonas veronii
    "714",  # Aggregatibacter actinomycetemcomitans
    "732",  # Aggregatibacter aphrophilus
    "358",  # Agrobacterium tumefaciens
    "28197",  # Aliarcobacter butzleri
    "33034",  # Anaerococcus prevotii
    "948",  # Anaplasma phagocytophilum
    "28264",  # Arcanobacterium haemolyticum
    "1392",  # Bacillus anthracis
    "1396",  # Bacillus cereus
    "1402",  # Bacillus licheniformis
    "774",  # Bartonella bacilliformis
    "56426",  # Bartonella clarridgeiae
    "807",  # Bartonella elizabethae
    "33045",  # Bartonella grahamii
    "38323",  # Bartonella henselae
    "803",  # Bartonella quintana
    "518",  # Bordetella bronchiseptica
    "519",  # Bordetella parapertussis
    "520",  # Bordetella pertussis
    "40834",  # Borrelia duttonii
    "140",  # Borrelia hermsii
    "47466",  # Borrelia miyamotoi
    "141",  # Borrelia parkeri
    "44449",  # Borrelia recurrentis
    "142",  # Borrelia turicatae
    "139",  # Borreliella burgdorferi
    "1674146",  # Borreliella mayonii
    "159",  # Brachyspira hyodysenteriae
    "1393",  # Brevibacillus brevis
    "235",  # Brucella abortus
    "36855",  # Brucella canis
    "29459",  # Brucella melitensis
    "29461",  # Brucella suis
    "95486",  # Burkholderia cenocepacia
    "292",  # Burkholderia cepacia
    "13373",  # Burkholderia mallei
    "28450",  # Burkholderia pseudomallei
    "195",  # Campylobacter coli
    "200",  # Campylobacter curvus
    "196",  # Campylobacter fetus
    "197",  # Campylobacter jejuni
    "201",  # Campylobacter lari
    "2718",  # Cardiobacterium hominis
    "83558",  # Chlamydia pneumoniae
    "83554",  # Chlamydia psittaci
    "813",  # Chlamydia trachomatis
    "546",  # Citrobacter freundii
    "1496",  # Clostridioides difficile
    "1491",  # Clostridium botulinum
    "1502",  # Clostridium perfringens
    "1504",  # Clostridium septicum
    "1513",  # Clostridium tetani
    "1717",  # Corynebacterium diphtheriae
    "38301",  # Corynebacterium minutissimum
    "43770",  # Corynebacterium striatum
    "65058",  # Corynebacterium ulcerans
    "777",  # Coxiella burnetii
    "28141",  # Cronobacter sakazakii
    "413502",  # Cronobacter turicensis
    "636",  # Edwardsiella tarda
    "945",  # Ehrlichia chaffeensis
    "947",  # Ehrlichia ewingii
    "35795",  # Ehrlichia muris
    "539",  # Eikenella corrodens
    "1117645",  # Elizabethkingia anophelis
    "238",  # Elizabethkingia meningoseptica
    "550",  # Enterobacter cloacae
    "33945",  # Enterococcus avium
    "1351",  # Enterococcus faecalis
    "1352",  # Enterococcus faecium
    "1353",  # Enterococcus gallinarum
    "1648",  # Erysipelothrix rhusiopathiae
    "562",  # Escherichia coli
    "1260",  # Finegoldia magna
    "263",  # Francisella tularensis
    "859",  # Fusobacterium necrophorum
    "851",  # Fusobacterium nucleatum
    "2702",  # Gardnerella vaginalis
    "197575",  # Haemophilus aegyptius
    "727",  # Haemophilus influenzae
    "729",  # Haemophilus parainfluenzae
    "210",  # Helicobacter pylori
    "502",  # Kingella denitrificans
    "504",  # Kingella kingae
    "548",  # Klebsiella aerogenes
    "39824",  # Klebsiella granulomatis
    "571",  # Klebsiella oxytoca
    "573",  # Klebsiella pneumoniae
    "447",  # Legionella bozemanae
    "451",  # Legionella micdadei
    "446",  # Legionella pneumophila
    "100053",  # Leptospira alexanderi
    "28452",  # Leptospira alstonii
    "174",  # Leptospira borgpetersenii
    "173",  # Leptospira interrogans
    "29507",  # Leptospira kirschneri
    "408139",  # Leptospira kmetyi
    "1137606",  # Leptospira mayottensis
    "28182",  # Leptospira noguchii
    "28183",  # Leptospira santarosai
    "28184",  # Leptospira weilii
    "1639",  # Listeria monocytogenes
    "28227",  # Malacoplasma penetrans
    "2098",  # Metamycoplasma hominis
    "2124",  # Metamycoplasma salivarium
    "408",  # Methylorubrum extorquens
    "480",  # Moraxella catarrhalis
    "477",  # Moraxella lacunata
    "478",  # Moraxella nonliquefaciens
    "582",  # Morganella morganii
    "1764",  # Mycobacterium avium
    "1767",  # Mycobacterium intracellulare
    "1768",  # Mycobacterium kansasii
    "1769",  # Mycobacterium leprae
    "480418",  # Mycobacterium lepromatosis
    "1780",  # Mycobacterium malmoense
    "1781",  # Mycobacterium marinum
    "1783",  # Mycobacterium scrofulaceum
    "1784",  # Mycobacterium simiae
    "1787",  # Mycobacterium szulgai
    "1773",  # Mycobacterium tuberculosis
    "1809",  # Mycobacterium ulcerans
    "1789",  # Mycobacterium xenopi
    "36809",  # Mycobacteroides abscessus
    "1774",  # Mycobacteroides chelonae
    "1788",  # Mycolicibacter terrae
    "1766",  # Mycolicibacterium fortuitum
    "1772",  # Mycolicibacterium smegmatis
    "59750",  # Mycolicibacterium wolinskyi
    "2097",  # Mycoplasmoides genitalium
    "2104",  # Mycoplasmoides pneumoniae
    "55603",  # Mycoplasmopsis caviae
    "2115",  # Mycoplasmopsis fermentans
    "485",  # Neisseria gonorrhoeae
    "487",  # Neisseria meningitidis
    "951",  # Neorickettsia sennetsu
    "1824",  # Nocardia asteroides
    "37326",  # Nocardia brasiliensis
    "37329",  # Nocardia farcinica
    "37330",  # Nocardia nova
    "1823",  # Nocardia otitidiscaviarum
    "784",  # Orientia tsutsugamushi
    "1490",  # Paraclostridium bifermentans
    "33033",  # Parvimonas micra
    "747",  # Pasteurella multocida
    "1258",  # Peptoniphilus asaccharolyticus
    "1261",  # Peptostreptococcus anaerobius
    "38293",  # Photobacterium damselae
    "703",  # Plesiomonas shigelloides
    "837",  # Porphyromonas gingivalis
    "43767",  # Prescottella equi
    "28125",  # Prevotella bivia
    "28131",  # Prevotella intermedia
    "28132",  # Prevotella melaninogenica
    "584",  # Proteus mirabilis
    "102862",  # Proteus penneri
    "585",  # Proteus vulgaris
    "126385",  # Providencia alcalifaciens
    "587",  # Providencia rettgeri
    "287",  # Pseudomonas aeruginosa
    "294",  # Pseudomonas fluorescens
    "303",  # Pseudomonas putida
    "329",  # Ralstonia pickettii
    "35788",  # Rickettsia africae
    "786",  # Rickettsia akari
    "787",  # Rickettsia australis
    "788",  # Rickettsia canadensis
    "781",  # Rickettsia conorii
    "42862",  # Rickettsia felis
    "35789",  # Rickettsia helvetica
    "37816",  # Rickettsia honei
    "35790",  # Rickettsia japonica
    "33991",  # Rickettsia montanensis
    "35792",  # Rickettsia parkeri
    "782",  # Rickettsia prowazekii
    "783",  # Rickettsia rickettsii
    "35793",  # Rickettsia sibirica
    "785",  # Rickettsia typhi
    "2047",  # Rothia dentocariosa
    "54736",  # Salmonella bongori
    "28901",  # Salmonella enterica
    "615",  # Serratia marcescens
    "38313",  # Shewanella algae
    "621",  # Shigella boydii
    "622",  # Shigella dysenteriae
    "623",  # Shigella flexneri
    "624",  # Shigella sonnei
    "1280",  # Staphylococcus aureus
    "29388",  # Staphylococcus capitis
    "1282",  # Staphylococcus epidermidis
    "1283",  # Staphylococcus haemolyticus
    "28035",  # Staphylococcus lugdunensis
    "283734",  # Staphylococcus pseudintermedius
    "33028",  # Staphylococcus saccharolyticus
    "29385",  # Staphylococcus saprophyticus
    "40324",  # Stenotrophomonas maltophilia
    "34105",  # Streptobacillus moniliformis
    "1311",  # Streptococcus agalactiae
    "1328",  # Streptococcus anginosus
    "1333",  # Streptococcus criceti
    "1334",  # Streptococcus dysgalactiae
    "1336",  # Streptococcus equi
    "1335",  # Streptococcus equinus
    "1302",  # Streptococcus gordonii
    "1346",  # Streptococcus iniae
    "28037",  # Streptococcus mitis
    "1309",  # Streptococcus mutans
    "1303",  # Streptococcus oralis
    "1313",  # Streptococcus pneumoniae
    "1314",  # Streptococcus pyogenes
    "1305",  # Streptococcus sanguinis
    "1310",  # Streptococcus sobrinus
    "1307",  # Streptococcus suis
    "13276",  # Suttonella indologenes
    "28112",  # Tannerella forsythia
    "158",  # Treponema denticola
    "160",  # Treponema pallidum
    "69710",  # Treponema vincentii
    "2039",  # Tropheryma whipplei
    "1661",  # Trueperella pyogenes
    "134821",  # Ureaplasma parvum
    "2130",  # Ureaplasma urealyticum
    "663",  # Vibrio alginolyticus
    "666",  # Vibrio cholerae
    "670",  # Vibrio parahaemolyticus
    "672",  # Vibrio vulnificus
    "630",  # Yersinia enterocolitica
    "632",  # Yersinia pestis
    "633",  # Yersinia pseudotuberculosis
    "730",  # [Haemophilus] ducreyi
]

CZID_BAC_SET = frozenset(CZID_BAC_LIST)

if __name__ == "__main__":
    sys.exit(main())


